...
### M15: 增强监控面板
- **决策**：Dashboard analytics 从 rollout JSONL 文件解析（非内存计数器），提取到独立模块 `src/server/dashboard-analytics.ts`。前端采用 4-tab 布局（总览 / Token 成本 / 记忆 / 工具使用），使用 SVG 绘制 sparkline 和柱状图（不引入 D3/Chart.js）。数据通过 2 秒轮询更新。
- **为什么**：rollout JSONL 文件已作为审计日志持续写入，从中提取 analytics 避免维护独立的 analytics 数据库或内存计数器。4-tab 布局将信息分组，避免单页信息过载。SVG 渲染是轻量级选择，不增加打包体积。Prometheus 指标（`/metrics` endpoint）提供机器可读的监控数据，Dashboard 面板提供人类可读的可视化。
- **Trade-off**：解析 JSONL 文件是 O(n) 复杂度，线程和事件很多时 dashboard endpoint 可能变慢。缓解策略：只读最近 5 个 rollout 文件，事件缓冲上限 60 条。前端 2 秒轮询可能产生不必要的请求，但 WebSocket 推送 dashboard 更新会增加实现复杂度。
### M16: 多 Agent 协作
...
### M21: 多模态支持
- **决策**：图片上传使用独立 HTTP POST endpoint
...
### M21: 多模态支持
- **决策**：图片上传使用独立 HTTP POST endpoint（而非通过 WebSocket 传输 base64），前端先上传图片获取服务器路径，再在 `turn/start` 时传递路径列表。LLM 调用时将图片路径转为 `image_url` ContentPart，构建多模态消息。前端使用本地 `URL.createObjectURL` 做即时预览，发送成功后释放。后端使用 `src/upload/index.ts` 处理上传，原生 multipart 解析零外部依赖，10MB 大小限制 + MIME 类型校验，文件存储在 `chitu-data/uploads/`。
- **为什么**：WebSocket 传输 base64 图片会阻塞 JSON-RPC 通道（大图片可达数 MB）。HTTP 上传是标准做法，支持流式传输和进度显示。服务器已有静态文件服务（`/uploads/*`），图片路径直接复用。`ContentPart[]` 类型已在 `llm/client.ts` 中定义（对齐 OpenAI multimodal API），后端只需在 Agent Loop 中替换最后一条用户消息的 content 即可。Agent Loop 通过 `multimodalContent` 配置参数接收图片列表，将最后一条用户消息替换为多模态 ContentPart 数组。
- **Trade-off**：上传需要额外 HTTP 请求（与 WebSocket 通道分离），但避免了 WebSocket 消息体积问题。图片目前只在用户消息中展示，Assistant 回复中的图表生成（Mermaid/PlantUML）留给后续 milestone。前端限制最多 5 张、单张最大 10MB，防止滥用。
