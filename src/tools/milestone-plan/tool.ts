...
export const milestonePlanTool: Tool = {
  name: 'milestone_plan',
  description: `Read and manage milestone-driven implementation plans from plans.md. Commands: "read" (full plan), "next" (get next pending milestone), "start" (mark milestone in_progress), "complete" (mark completed + auto git checkpoint), "fail" (mark failed). Always check for a milestone plan before starting work on multi-step tasks.`,
  parameters: {
    type: 'object',
    properties: {
      command: {
        type: 'string',
        enum: ['read', 'next', 'start', 'complete', 'fail'],
        description: 'The milestone command to execute',
      },
      milestoneId: {
        type: 'string',
        description: 'The milestone ID (e.g. "M1"). Required for start/complete/fail.',
      },
      explanation: {
        type: 'string',
