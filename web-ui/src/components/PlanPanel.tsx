...
export function PlanPanel() {
  ...
  const allCompleted = currentPlan.every((s) => s.status === 'completed')
          ...
          {currentPlan.map((step: PlanStep, i: number) => {
            ...
            return (
              <div key={i} className="flex items-center gap-2">
                ...
                <span className={`text-xs ${step.status === 'completed' ? 'text-[#888] line-through' : step.status === 'in_progress' ? 'text-white font-medium' : 'text-[#aaa]'}`}>
                  {step.step}
                </span>
              </div>
            )
